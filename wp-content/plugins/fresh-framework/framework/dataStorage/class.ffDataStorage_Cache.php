...
class ffDataStorage_Cache extends ffBasicObject implements ffIDataStorage {
	...
	const CACHING_HASH_FILE_NAME = 'hash_info';
	...
	private function _touchLastDeletion( $namespace ) {
		...
		$this->_getFileSystem()->putContents( $checkFilePath, ffDataStorage_Cache::CACHING_LAST_DELETION_FILE_CONTENT, 0755 );
	}
	...
	public function getAllOptionNames( $namespace ) {
		...
		if( !is_array( $allFilesInNamespace ) ) {
			return array();
		}
	...
	public function getLastHashForOption( $namespace, $name, $extension = 'frs' ) {
		$hashFile = $this->getOption( $namespace, ffDataStorage_Cache::CACHING_HASH_FILE_NAME );

		if( empty( $hashFile ) ) {
			return null;
		}

		$hashData = json_decode( $hashFile, true );
		$fullName = $name . $extension;

		if( isset( $hashData[ $fullName] ) ) {
	...
	public function isHashValidForOption( $namespace, $name, $hash, $extension = 'frs' ) {
		return $this->getLastHashForOption( $namespace, $name, $extension ) == $hash;
	}
	...
	private $_hashDataWriting = array();

	public function setLastHashForOption( $namespace, $name, $hash, $extension = 'frs' ) {
		$hashFile = $this->getOption( $namespace, ffDataStorage_Cache::CACHING_HASH_FILE_NAME );
		...
		$hashData[ $fullName ] = $hash;

		$hashFile = json_encode( $hashData );
		$this->setOption( $namespace, ffDataStorage_Cache::CACHING_HASH_FILE_NAME, $hashFile );
	}

	public function getOptionWithHash( $namespace, $name, $hash, $extension = 'frs' ) {
		if( $this->isHashValidForOption( $namespace, $name, $hash, $extension ) ) {
			return $this->getOption( $namespace, $name, $extension );
		} else {
			return null;
		}
	}

	public function setOptionJSON( $namespace, $name, $value, $extension = 'frs' ) {
		$value = json_encode( $value );
		return $this->setOption( $namespace, $name, $value, $extension );
	}

	public function setOptionWithHash( $namespace, $name, $value, $hash, $extension = 'frs' ) {
		$this->setOption( $namespace, $name, $value, $extension);
		$this->setLastHashForOption( $namespace, $name, $hash, $extension );
	}
	...
	public function setOption( $namespace, $name, $value, $extension = 'frs') {
		...
		$this->_getFileSystem()->putContents( $fullPath, $value, 0755);
	}

...
frslib.provide('frslib.options.template');
	...
	frslib.provide('frslib.options.ajaxDataGathering');

	frslib.options.ajaxDataGathering.getPosts = function( postType ) {
		if( postType == undefined ) { postType = 'page' };
...
/**********************************************************************************************************************/
    ...
    frslib.options.functions.initOneOptionSet = function( $optionsJSWrapper, doNotMakeAnimation ) {
		...
		if( $dataWrapper.find('.ff-options-structure-js').data('data-json') != undefined ) {
			structure = $dataWrapper.find('.ff-options-structure-js').data('data-json');
		}

		// console.log( $dataWrapper.find('.ff-options-data-js').attr('data-json') );
		if( $dataWrapper.find('.ff-options-data-js').data('data-json') != undefined ) {
			data = $dataWrapper.find('.ff-options-data-js').data('data-json');
		}
		...
		var optionsBlockCallback = $dataWrapper.find('.ff-options-structure-js').data('data-block-callback');

		if( optionsBlockCallback != undefined ) {
			printerBoxed.walker.callbackGetBlockItem = optionsBlockCallback;
		}
        ...
        if( doNotMakeAnimation == undefined ) {
        ...
        $optionsJSWrapper.find('.ff-all-items-opened').each(function(){

            $(this).children('li').each(function(){

                $(this).removeClass('ff-repeatable-item-closed').addClass('ff-repeatable-item-opened');

                $(this).children('.ff-repeatable-content').css('display', 'block');
        ...
        setTimeout(function(){
            ...
            frslib.options.functions.initParticularOptionsFromSelector( $optionsJSWrapper );

        }, 1000);

        var printCopyAndPaste = true;

        if( $optionsJSWrapper.attr('data-print-copy-and-paste') == 'false' ) {
            printCopyAndPaste = false;
        }

        if( $optionsJSWrapper.find('.ff-copy-and-paste').size() > 0 ) {
            $optionsJSWrapper.find('.ff-copy-and-paste').remove();
        }

        if( printCopyAndPaste == true ) {

            var html = '';

            html += '<div class="ff-copy-and-paste">';
            html += '<br>';
            html += '<input type="submit" style="" class="ff-onepage-copy-to-clipboard button-primary" value="Copy To Clipboard">';
            html += '&nbsp;';
            html += '<input type="submit" style="" class="ff-onepage-paste-from-clipboard button-primary" value="Erase and Paste From Clipboard">';
            html += '</div>';

            $optionsJSWrapper.find('.ff-options-js').after( html );


            $optionsJSWrapper.find('.ff-onepage-copy-to-clipboard').click(function(){

                var $form = $optionsJSWrapper.find('.ff-options-js');
                var data = {};
                var dataJSON = '';

                data.normalizedForm = frslib.options.template.functions.normalizeFast( $form );
                dataJSON = JSON.stringify( data );

                var suceed = frslib.clipboard.copyTo( dataJSON );

                if( suceed ) {
                    alert( 'Data are in clipboard!');
                }
            ...
            $optionsJSWrapper.find('.ff-onepage-paste-from-clipboard').click(function(){
                var prefix = $optionsJSWrapper.find('.ff-options-js-data-wrapper').find('.ff-options-prefix').html();

                var jsonContent = window.prompt('Please paste the content of your page here', '');
                ...
                if( data != null && data['normalizedForm'] == undefined ) {
                    alert( 'Not valid format');
                    return false;
                }

                if( data['normalizedForm'][prefix] == undefined ) {
                    alert( 'Not valid format');
                    return false;
                }

                var realData = data['normalizedForm'][prefix];


                var serializedData = JSON.stringify( realData );

                $optionsJSWrapper.find('.ff-options-js-data-wrapper .ff-options-data-js').html( serializedData );


                frslib.options.functions.initOneOptionSet($optionsJSWrapper);
        ...
        setTimeout( function(){
            ...
            var printerBoxed2 = frslib.options.walkers.printerBoxed();
			printerBoxed2.walker.isShadowPrinting = true;
			...
			if( optionsBlockCallback != undefined ) {
				printerBoxed2.walker.callbackGetBlockItem = optionsBlockCallback;
			}

			printerBoxed2.setStructureString( structure );
			// printerBoxed2.setDataString( data );
			printerBoxed2.setPrefix( prefix );
			printerBoxed2.walker.ignoreData = true;
			printerBoxed2.walker.ignoreHideDefault = true;

            printerBoxed2.walk();

            $optionsJSWrapper.data('stored-structure-html', printerBoxed2 );


        }, 150);

        frslib.callbacks.doCallback( 'initOneOptionSet', $optionsJSWrapper)
    }



    frslib.options.functions.initParticularOptionsFromSelector = function( $data ) {

         var $firstLevels = $data.findButNotInside('.ff-repeatable');
		...
		$data.find('.ff-options__tinymce').each(function(){
			...
			tinyMCE.init({
				...
				theme:"advanced",
				skin:"wp_theme",
				editor_selector : ".ff-options__tinymce"
			});

		});


		$data.find('.ff-option-tax-type-selector').each(function(){
			...
			frslib.options.select_content_type.getTaxByType(taxType, function( data ) {
				...
				$self.find('option').each(function(){
					...
					if( value.indexOf('||tax-s||') == -1 && value.length > 0 ) {
						value = '||tax-s||' + value + '||tax-e||';
				...
				$self.select2({dropdownCssClass:'ff-select2'});
			});

		});

		$data.find('.ff-option-post-type-selector').each(function(){

			var postType = $(this).attr('data-post-type');
			var $self = $(this);

			frslib.options.select_content_type.getPostsByType(postType, function( data ) {
				...
				$(data).each(function(){
					...
					newData += '<option value="' + postId + '--||--' + postName +'">' + postName + '</option>';

				});

				$self.parents('.ff-select2-wrapper').find('.spinner').remove();

				$self.html( $self.html() + newData );
				...
				$self.select2({containerCssClass	: 'ff-select2', dropdownCssClass:'ff-select2'});

			});

		});

        $data.find('.ff-select2-wrapper').each(function(){
        ...
        if( $data.find('.ff-revolution-slider-select-content').size() > 0 ) {
                var htmlContent = $data.find('.ff-revolution-slider-select-content').html();

                $data.find('.ff-revolution-slider-selector').html( htmlContent );

                $data.find('.ff-repeatable').children('.ff-repeatable-item').find('.ff-revolution-slider-selector').each(function(){
        ...
        setTimeout(function(){
            if( $data.find('.ff-datepicker').datepicker != undefined ) {
                $data.find('.ff-datepicker').datepicker();
            }
            ...
            //jQuery(".ff-default-wp-color-picker").wpColorPicker();
            	...
            	$data.find(".ff-default-wp-color-picker").each(function(){
    ...
    frslib.options.functions.initParticularOptions = function() {
        ...
        setTimeout(function(){
            if( $('.ff-datepicker').datepicker != undefined ) {
		...
		$('.ff-hiding-box-normal').each(function(){
    ...
    $(document).on('click', '.ff-repeatable-add-above-js, .ff-repeatable-add-below-js', function(e){

		var directionToAdd = 'below';
		if( $(this).hasClass('ff-repeatable-add-above') ) {
			directionToAdd = 'above';
		}
        ...
        var isBelow = $(this).hasClass('ff-repeatable-add-below') ? true : false;
...
//return;
        ...
        var insertTemplate = function( $li ) {
				...
				$li.find('.ff-hide-default').remove();
                ...
                $li.find('.ff-select2-wrapper').each(function(){

                    var value = $(this).find('.ff-select2-value-wrapper').find('input').val();

					if( value == undefined ) {
						var valueSplitted = undefined;
					}
				 	else {
						var valueSplitted = value.split('--||--');
					}
			...
			$li.parents('.ff-options-js-wrapper:first').trigger('opt-changed', ['insert', $li]);

			frslib.callbacks.doCallback('options-section-inserted', $li );


			if( $parentUl.hasClass('ff-all-items-opened') ) {
				$li.removeClass('ff-repeatable-item-closed').addClass('ff-repeatable-item-opened');
				$li.find('.ff-repeatable-content:first').css('display', 'block');
			}
        ...
        if( $parentUl.hasClass('ff-section-picker-advanced') ) {
            ...
            frslib._classes.modalWindowColorSectionPicker.open( function( selectedSectionID ){
                ...
                var $newLi = $( $li.outerHtml() );

                insertTemplate( $newLi );

            });
        /*----------------------------------------------------------*/
        /* NORMAL SECTION PICKER
        /*----------------------------------------------------------*/
        } else if( $('.ffb-modal-origin').size() > 0 ) {
					...
					content: {
						text:
							function(){
								...
								var toReturn = '';
								toReturn += '<ul class="ff-popup-qtip-wrapper" style="background-color:white;">';
								toReturn += buttons;
								toReturn += '</ul>';
								...
								$html.on('click', '.ff-popup-button-wrapper', function(){
            ...
            if( childSections.length == 1 ) {
               var section = childSections[0];
				...
				if( $('.ffb-modal__action-save-all').size() > 0 ) {
				$('.ffb-modal__action-save-all').qtip({
					content: {
						text:
							function(){
				...
				$('.ffb-modal__action-save-all').qtip('toggle', true);
...
frslib.options.template.hasBeenInited = false;
frslib.options.template.init = function() {
	if( frslib.options.template.hasBeenInited ) {
		return;
	}

	frslib.options.template.hasBeenInited = true;
...
frslib.options.template.init_one_sortable = function( $ul_ff_repeatable ) {
	...
	$ul_ff_repeatable.filter(':not(.ff-toggle-box)').sortable({
		handle: '.ff-repeatable-handle:not(.ff-repeatable-handle-toggle-box)',
		...
		update: function(){
			...
			$(this).parents('.ff-options-js-wrapper:first').trigger('opt-changed', ['sorted'] );
			frslib.callbacks.doCallback('opt-changed-sorted', $(this));

		},

        // @TODO TOMAS - UNCOMMENT THIS
        // start: function() {
        //     $(this).sortable('refresh'); // ADDITION FOR FF CANVAS BUILDER. needed because when you drag a long element downwards, it does work very good. this helps recalculate everything.
        // },
...
$(document).on('click', '.ff-repeatable-duplicate', function(){

	/*----------------------------------------------------------*/
	/* COPY PART
 	/*----------------------------------------------------------*/

	var $parent = $(this).parents('.ff-repeatable:first')
	var $item = $(this).parents('.ff-repeatable-item:first');

	var itemOuterHtml = $item.outerHtml();
	var $newItem = $( itemOuterHtml );
	var $newItemInputs = $newItem.find('input[name], select[name], textarea[name]');

	$item.find('input[name], select[name], textarea[name]').each(function(index,input){
		var value = $(this).attr('value');
		...
		if( $(this).attr('type') == 'checkbox' ){
			...
			$newItemInputs.eq(index).attr('data-value', 'not-checked');
			if( $(this).is(':checked') ) {
				$newItemInputs.eq(index).attr('data-value', 'checked');
			}
		} else {

			$newItemInputs.eq( index).prop('value', value);
			$newItemInputs.eq(index).attr('data-value', value);
		}
	});

	var itemHTML = $newItem.outerHtml();

	/*----------------------------------------------------------*/
	/* PASTE PART
	/*----------------------------------------------------------*/

	var content = itemHTML;

	var $itemToPaste = $( content );


	$itemToPaste.find('.ff-popup-open').removeClass('ff-popup-open');

	$itemToPaste.find('[data-value]').each(function(){
		...
		if( $(this).attr('type') == 'checkbox' ) {
			...
			$(this).val('value', $(this).attr('data-value') );
	...
	$itemToPaste.find('.ff-repeatable-item-popup-opened').removeClass('ff-repeatable-item-popup-opened');
	$itemToPaste.find('.ff-repeatable-item-hover').removeClass('ff-repeatable-item-hover');

	$itemToPaste.removeClass('ff-repeatable-item-hover').removeClass('ff-repeatable-item-popup-opened');

	$itemToPaste.find('.ff-select2-real-wrapper').html();

	// var $parent = $(this).parents('.ff-repeatable:first');
	var $item = $(this).parents('.ff-repeatable-item:first');

	if( $itemToPaste.attr('data-current-section-route') != $parent.attr('data-current-section-route') ) {
		alert('Not compatible data format');
		return false;
	}

	$item.after( $itemToPaste );

	$(this).parents('.ff-options-js-wrapper:first').trigger('opt-changed', ['pasted'] );

	frslib.callbacks.doCallback('options-section-inserted', $itemToPaste );
});


$(document).on('click', '.ff-repeatable-copy', function(){
    ...
    frslib.clipboard.copyTo( itemHTML );

    alert('Section has been copied to clipboard');
...
$(document).on('click', '.ff-repeatable-paste', function(){
    var content = window.prompt('Please paste the section content here', '');

    var $item = $( content );


    $item.find('.ff-popup-open').removeClass('ff-popup-open');
    ...
    $item.find('.ff-repeatable-item-popup-opened').removeClass('ff-repeatable-item-popup-opened');
    $item.find('.ff-repeatable-item-hover').removeClass('ff-repeatable-item-hover');

    $item.removeClass('ff-repeatable-item-hover').removeClass('ff-repeatable-item-popup-opened');

    $item.find('.ff-select2-real-wrapper').html();

    var $parent = $(this).parents('.ff-repeatable:first');
    var $itemAfter = $(this).parents('.ff-repeatable-item:first');

    if( $item.attr('data-current-section-route') != $parent.attr('data-current-section-route') ) {
        alert('Not compatible data format');
        return false;
    }

    $itemAfter.after( $item );
...
$(document).on('click', '.ff-popup-button', function( e ){
    $(this).parents('.ff-popup-container:first').removeClass('ff-popup-open');

    $(this).parents('.ff-repeatable-item:first').removeClass('ff-repeatable-item-hover');


    e.stopPropagation();
    return false;
    //    alert('click');
    //$( '.ff-popup-container' ).trigger('click');
...
$(document).on('click',frslib.options.template.selectors.repeatable_button_remove, function(){
    ...
    var confirmation = confirm('Do you really want to remove this section?');
	...
	if( $parent_ul.hasClass('ff-can-be-empty') ) {
		canBeEmpty = true;
	}

	// if( canBeEmpty  && number_of_siblings_li == 1 ){
	// 	$parent_ul.find('.ff-repeatable-item-empty-add:first').css('display', 'block');
	// }

	if( !canBeEmpty && ( number_of_siblings_li == 1 ) && ! enable_delete_all_repeatable_items ) {
	...
	$parent.animate({ height: 'toggle', opacity: 'toggle' }, 400, function(){
		...
		$parent_ul.parents('.ff-options-js-wrapper:first').trigger('opt-changed', ['removed'] );
		frslib.callbacks.doCallback('opt-changed-remove', $parent_ul );
...
$(document).on('click',frslib.options.template.selectors.repeatable_button_handle, function(){
    ...
    var workAsAccordion = false;

    if( $parent_ul.hasClass('ff-work-as-accordion')) {
        workAsAccordion = true;
    }
    ...
    if ( $(this).closest('.ffb-options').length ) {
        speed = 0;
    }
...
frslib.options.template.functions.normalizeFast = function( $form, returnForm, deleteOriginalForm, ignoreDefaultValues, escapeValues ) {

		if( escapeValues == undefined ) {
			escapeValues = false;
		}

        if( ignoreDefaultValues == undefined ) {
            ignoreDefaultValues = false;
        }

        var hasTemplateHolder = false;

        if( $form.find('.ff-repeatable-template-holder').size() > 0 ) {
            hasTemplateHolder = true;
        }

        if( deleteOriginalForm == undefined && returnForm != undefined ) {
            deleteOriginalForm = true;


        }


        if( deleteOriginalForm ) {

        }


    //$form.find('.ff-repeatable-template-holder').find('input, textarea').attr('name', '');

        //if( )

            //var self = this;
            //if( this.submitFormTimer == null ) {
            //     this.submitFormTimer = setTimeout(function(){
            ...
            if( $f.is('form') ) {
                ...
                $newForm.attr('method', $f.attr('method') );
                $newForm.attr('action', $f.attr('action') );
            }

            else if( $f.find('form').size() > 0 ) {
                $f2 = $f.find('form');

                $newForm.attr('method', $f2.attr('method') );
                $newForm.attr('action', $f2.attr('action') );
            }
            ...
            var setDataPointer = function( route, value ) {
            ...
            var currentLevel = 0;
                var counters = {};
            $f.find('.ff-repeatable, .ff-repeatable-item, input[name], select[name], textarea[name]').each(function(){

                if( $(this).hasClass('ff-checkbox-shadow') ) {
                            return;
                }

                if( hasTemplateHolder ) {
                    if( $(this).parents('.ff-repeatable-template-holder').size() > 0 ) {
                        return true;
                    }
                }
                ...
                if( $(this).hasClass('ff-repeatable') ) {
					...
					// console.log( $(this).attr('data-current-name-route') );

					var name = $(this).attr('data-current-name-route');
					var value = {};
				...
				// else {
					...
					/* SELECT 2
					/*----------------------------------------------------------*/
                   else  if( $(this).hasClass('ff-select2') ) {
                        var value = $(this).parents('.ff-select2-wrapper').find('.ff-select2-real-wrapper').find('.select2-container').select2('val');
                    ...
                    } else if ( $(this).prop('tagName') == 'SELECT' && $(this).parents('.ff-select2-wrapper').size() > 0 && $(this).siblings().size() == 0) {
						var value = $(this).attr('data-selected-value');
						if( value == '||tax-s||||tax-e||' ) {
							value = '';
						}
					...
					//TINY MCE
					else if( $(this).hasClass('ff-options__tinymce') ) {
						var id = $(this).attr('id');
                    ...
                    if( $(this).hasClass('ff-image') ) {
                        //value = value.split('"').join('_ffqt_');
                    }
                    ...
                    if( name.indexOf('-TEMPLATE-_-') != - 1 ) {
                        ...
                        for( var key in counters ) {
                            ...
                            name = name.replace('-_-'+key+'-TEMPLATE-_-', counters[key]);
                    ...
                    //if( defaultValue != undefined && value == defaultValue && ignoreDefaultValues == true ) {
                    //    return;
                    //}//ignoreDefaultValues
                     ...
                     if( returnForm ) {
                        ...
                        firstElementName = nameSplitted[0];
                        name = name.replace(firstElementName + '[', '');
                        var route =  name.split('][');
                        route.unshift( firstElementName );


                        setDataPointer( route, value );
                    }
        ...
        if( returnForm ) {
            ...
            $newForm.append('<input type="hidden" name="has-been-normalized" value="1" />');
            $newForm.find('input[type="submit"]').remove();
            $newForm.find('[name="submit"]').remove();

            return $newForm;

        } else {

            data['has-been-normalized'] = 1;
...
frslib.options.template.functions.normalize = function( $form, submit ) {
    ...
    function ffNormalizEscapeHtml(text) {
          return text
              .replace(/&/g, "&amp;")
              .replace(/</g, "&lt;")
              .replace(/>/g, "&gt;")
	...
	$form.find('input, select, textarea').each(function(i, o){
		...
		// NORMALIZING SELECTS
			...
			// SINGULAR
                ...
                //$(this).val( ffNormalizEscapeHtml( currentValue ) );
			}
		}
	...
	var number_of_sections = $formClonned.find('.ff-repeatable, .fftm__option-type--repeatable-init').size();
    ...
    $formClonned.find('input, select, textarea').each(function(i, o){
       //if( $(this).attr())
        var value = $(this).attr('data-value');
	...
	for( var i = 0; i < number_of_sections; i++) {
		var $repeat = $formClonned.find('.ff-repeatable:eq('+i+'), .fftm__option-type--repeatable-init:eq('+i+')');
		...
		$repeat.children('.ff-repeatable-item, .fftm__option-type--repeatable-item-init').each(function(){
...
/*	$('.ff-testing-option-form').submit(function(){
	...
	$('.ff-form-submit').click(function(){
        ...
        var $originalForm = $(this).parents('form:first')
        var $form = null;


        if( $originalForm.hasClass('ff-normalize-old-way') ) {
            $form = frslib.options.template.functions.normalize( $originalForm );//.submit();
        } else {
            $form = frslib.options.template.functions.normalizeFast( $originalForm, true,true);//.submit();
        }
...
$('.widget-control-actions').find('input[type="submit"]').click(function(){
	...
	var $normalizedForm = frslib.options.template.functions.normalizeFast( $form, true, true );
...
frslib.options.select2.initOne = function( $select2Wrapper) {
    $select2Wrapper.each(function(){
...
frslib.callbacks.addCallback( 'kkff_duplicate_after_clone', function( $newItem, $parent ){
	...
	$newItem.find( frslib.options.select2.selectors.main_wrapper ).each(function(i,o){
        ...
        setTimeout(function(){
            $this.find('.ff-select2-real-wrapper select.ff-select2').css('display', 'none !important').addClass('select2-offscreen');
        }, 1000);
...
frslib.options.select_content_type.init();
	...
	frslib.options.select_content_type.getPostsByType = function( postType, callback ) {
		var current_value = 'post-|-' + postType;

		if( frslib.options.select_content_type.fetched_data[ current_value ] == undefined ) {
			frslib.ajax.frameworkRequest( 'ffOptionsPrinterDataBoxGenerator', {'type':'select_content_type'}, { 'select_value' : current_value }, function( response ){
				frslib.options.select_content_type.fetched_data[ current_value ] = response;
				callback(frslib.options.select_content_type.fetched_data[ current_value ] );
			});
		} else {
			callback(frslib.options.select_content_type.fetched_data[ current_value ] );
		}
	...
	frslib.options.select_content_type.getTaxByType = function( taxType, callback ) {
		var current_value = 'tax-|-' + taxType;
...
(function(){
	...
	$(document).on('click','.ff-open-library-remove',function(e){
		if( $(this).parents('.ff-open-library-button-wrapper:first').hasClass('ff-open-image-library-button-wrapper') ) {
			...
			var $button = $(this).parents('.ff-open-icon-library-button-wrapper:first').find('.ff-open-library-button');

			$button.find('input').val('');
			$button.find('input').change();
			$button.find('.ff-open-library-button-preview').find('i').attr('class','');
			$button.parents('.ff-open-image-library-button-wrapper').addClass('ff-empty');
	...
	$(document).on('mouseenter', '.ff-open-icon-library-button-wrapper', function(){
		$(this).find('.ff-open-library-remove').css('visibility', 'visible');
	});

	$(document).on('mouseleave', '.ff-open-icon-library-button-wrapper', function(){
		$(this).find('.ff-open-library-remove').css('visibility', 'hidden');
	});

	//ff-open-icon-library-button-wrapper

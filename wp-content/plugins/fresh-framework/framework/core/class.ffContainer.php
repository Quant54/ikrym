...
/******************************************************************************/
	...
	public function getEnvironment() {
		if( $this->_environment == null ) {
			$this->_environment = new ffEnvironment( $this->getWPLayer() );
		}
	...
	public function getWPUser() {
		$this->getClassLoader()->loadClass('ffWPUser');

		$wpUser = new ffWPUser( $this->getWPLayer() );

		return $wpUser;
	}

	public function getFastSpringApi() {
		$this->getClassLoader()->loadClass('ffFastSpringApi');

		$envatoApi = new ffFastSpringApi( $this->getHttp(), $this->getRequest() );
	...
	public function getWPQueryHelper() {
		$this->getClassLoader()->loadClass('ffWPQueryHelper');

		$WPQueryHelper = new ffWPQueryHelper();

		return $WPQueryHelper;
	}

	public function getWPConfigEditor() {
		$this->getClassLoader()->loadClass('ffWPConfigEditor');

		$WPConfigEditor = new ffWPConfigEditor( $this->getWPLayer(), $this->getFileSystem() ) ;

		return $WPConfigEditor;
	}
	...
	public function getAdminNotices() {
		if( $this->_adminNotices == null ) {
			$this->_adminNotices = new ffAdminNotices( $this->getWPLayer() );
		}
	...
	public function getElementHelper() {
		$this->getClassLoader()->loadClass('ffElementHelper');

		$helper = new ffElementHelper();

		return $helper;
	}

    public function getMultiAttrHelper() {
        $this->getClassLoader()->loadClass('ffMultiAttrHelper');
        $helper = new ffMultiAttrHelper( $this->getWPLayer() );
	...
	private $_fwScriptLoader = null;

	public function getFrameworkScriptLoader() {
		if( $this->_fwScriptLoader == null ) {
			...
			$this->_fwScriptLoader = $frameworkScriptLoader;
		}

		return $this->_fwScriptLoader;
	}
    ...
    public function getJsTreeDataBuilder() {
        $this->getClassLoader()->loadClass('ffJsTreeDataBuilder');

        $jsTreeDataBuilder = new ffJsTreeDataBuilder( $this->createNewCollection() );

        return $jsTreeDataBuilder;
    }
	...
	public function getLessParser() {
		...
		var_dump( $cachingDir['basedir'].'/ZZZZZZ' );
		//die();
		$options =  array('cache_callback_set'=>'ff_less_ZZZZZZ', 'sourceMap'=>true);
		$less = new lessc_freshframework( );
		$less->setOptions($options);
		return $less;*/
		
		//$cachingDir = $this->getWPLayer()->wp_upload_dir();
		//$options =  array( 'cache_dir'=> $cachingDir['basedir'].'/ZZZZZZ', 'sourceMap'=>true);
		$less = new lessc_freshframework();
		//$less->setOptions($options);
		return $less;
	}
	...
	public function createNewCollectionAdvanced() {
		$this->getClassLoader()->loadClass('ffCollection');
		$this->getClassLoader()->loadClass('ffCollectionAdvanced');
		$collection = new ffCollectionAdvanced();
		return $collection;
	}

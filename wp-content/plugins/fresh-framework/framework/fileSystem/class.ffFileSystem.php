...
/******************************************************************************/
    ...
    public function getUrlFromPath( $absolutePath ) {
        ...
        $wpContentDir = $WPLayer->get_wp_content_dir();
        $wpContentUrl = $WPLayer->get_wp_content_url();

        $pathStripped = str_replace( $wpContentDir, '', $absolutePath);

        if( $pathStripped == $absolutePath ) {
            return false;
        }


        return( $wpContentUrl.$pathStripped  );
    }

    public function getAbsolutePathInTemplate( $relativePath, $returnUrlInsteadOfDir = false ) {
	...
	public function getThemePath( $relativePath = null ) {
		$path = $this->_getWPLayer()->get_template_directory() . $relativePath;
	 ...
	 * like a boss :-)
	 */
	public function reqFileInChildTheme( $ffRelativePath, $ffArgumentsArray = null ){
		$this->requireFileInChildTheme( $ffRelativePath, $ffArgumentsArray );
	}
	...
	public function serveFile( $file, $dieAtTheEnd = true ) {
	...
	public function delete( $path, $recursive = false, $type = false ) {
		return $this->_getWPFileSystem()->delete( $path, $recursive, $type );
	}


	public function findFileFromUrl( $url ) {
		$WPLayer = $this->_getWPLayer();
		if( $WPLayer->is_multisite() ) {
//			var_Dump( $url );
//			die();
			// TODO FOR WORDPRESS MU
			$homeUrl = $this->_getWPLayer()->get_site_url(1);
			if( FALSE !== strpos( $homeUrl, '?lang' ) ){
				$homeUrl = substr( $homeUrl, 0, strpos( $homeUrl, '?lang' ) );
			}
			...
			$wpContentDirPos = strpos( $withoutBase, '/wp-content');

			if( $wpContentDirPos !== false ) {
				$withoutBase = substr( $withoutBase, $wpContentDirPos );
			}
	...
	public function getFilesFromDirRecursive( $path ) {
		$toReturn = array();

		return $this->_dirlistRecursive( $path, $toReturn );
	}

	private function _dirlistRecursive( $path, $toReturn ) {
		$contentOfDir = $this->dirlist( $path );

		foreach( $contentOfDir as $oneItem ) {
			if( $oneItem['type'] == 'd' ) {
				$toReturn = $this->_dirlistRecursive( $path . '/' . $oneItem['name'], $toReturn );
			} else {

				$oneItem['dir'] = $path;
				$oneItem['path'] = $path . '/' . $oneItem['name'];
	...
	public function getDirUpgradeAndMakeSurItsWritable() {
		$upgradeDir = $this->getDirUpgrade();

		if( !$this->fileExists( $upgradeDir ) ) {
			$this->makeDir( $upgradeDir, 0777 );
		}
	...
	public function enableMaintenanceMode( $timeEnabled = null ) {
		if( $timeEnabled == null ) {
			$timeEnabled = time();
		}

		$maintenanceFileTemplate = '<?php $upgrading = ' . $timeEnabled . '; ?>';

		$rootDir = $this->getAbsPath();
		$maintenancePath = $rootDir . '.maintenance';

		$this->putContents( $maintenancePath, $maintenanceFileTemplate);
	}

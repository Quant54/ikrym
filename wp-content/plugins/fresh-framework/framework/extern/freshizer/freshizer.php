...
if( !class_exists('fImg') ) {
	...
	require_once 'freshizer_facade.php';
	/*******************************************************************************
...
// Optimal is approx 400 - 500 hits
	...
	class blFileSystem
	{
		...
		public function saveImage($image, $path, $quality )
		{
			$pinfo = pathinfo($path);
			$ext = $pinfo['extension'];
			$return = null;
...
//					die();
	...
	class blImgCache
	{
		...
		private function _saveAfterEdit()
		{
			$this->saveCacheFile();
			$this->_loadCacheFile();
			$this->_unparseCacheFile();
		...
		private function _unparseCacheFile()
		{
			if ($this->_getCacheFileParsed() != '') {
				...
				if( ! is_object( $cacheFileContentUnparsed ) ){
					$cacheFileContentUnparsed = new stdClass();
					$cacheFileContentUnparsed->hitsAfterLastDelete = 0;
				}
	...
	class fImgNamer
	{
		...
		public function getNewImageName($oldUrl, $width, $height = false, $crop = false, $remote = false, $quality = 90)
		{
			$newUrl = '';

			$partRemote = ($remote) ? 'remote/' : '';
			$partWidth = '-' . absint($width);
			$partHeight = ($height) ? '-' . absint($height) : '';
			$partCrop = ($crop) ? '-c' : '';
			$partQuality = '-' . $quality;
		...
		public function getNewImageUrl($oldUrl, $width, $height = false, $crop = false, $remote = false, $quality = 90)
		{
			/**
	...
	class fImgPathPredictor_Multisite extends fAImgPathPredictor implements fIImgPathPredictor
	{
		...
		protected function _predictionJunction()
		{
			...
			} else if( isset($wpUploadDir['url']) && strpos( $this->_getImgUrl(), $wpUploadDir['url'] ) !== false ) {
				$this->_predictCustomUploadDir();
			}

		}


		protected function _predictCustomUploadDir() {

			$wpUploadDir = wp_upload_dir();
			$imageUrl = $this->_getImgUrl();

			$imgUploadDirSplited = explode($wpUploadDir['url'], $this->_getImgUrl());
			...
			$newRelPath2 = $wpUploadDir['path'] . $imgAfterUploadDir;
			if (file_exists($newRelPath)) {
				$this->_setPredictedPath($newRelPath);
			} else if ( file_exists( $newRelPath2 ) ) {
				$this->_setPredictedPath($newRelPath2);
			}
		}
		...
		protected function _predictThemes()
		{
			$splitedUrl = explode('themes/', $this->_getImgUrl());
	...
	class fImgResizer
	{
		...
		public function resize(fImgData $imgData)
		{

			$imageOld = $this->_openImage($imgData->old->path);
			$orig = $this->_getImgDimensions($imgData->old->path);

			if ($imgData->new->width == 0) {
				$ration = $orig->width / $orig->height;
				$imgData->new->width = $ration * $imgData->new->height;
			}
			...
			$this->_getFileSystem()->saveImage($imageNew, $imgData->new->path, $imgData->new->quality);
			$this->_getFileSystem()->saveImage($imageNewRetina, $this->_getRetinaPath($imgData->new->path), $imgData->new->quality );
	...
	class fImgResizerCalculator
	{
		public function calculateNewDimensions($orig_w, $orig_h, $dest_w, $dest_h, $crop = false)
		{
			if ($dest_w == 0 && $dest_h == 0) {
				$to_return = array();
				$to_return['src']['x'] = (int)0;
				$to_return['src']['y'] = (int)0;
				$to_return['src']['w'] = (int)$orig_w;
				$to_return['src']['h'] = (int)$orig_h;

				$to_return['dst']['x'] = 0;
				$to_return['dst']['y'] = 0;
				$to_return['dst']['w'] = (int)$orig_w;
				$to_return['dst']['h'] = (int)$orig_h;
	...
	class freshizerErrorHandler{

		/**
		 * @var freshizerErrorHandler
		 */
		...
		public static function getInstance() {
			if (self::$_instance == null) {
				self::$_instance = new freshizerErrorHandler();
			}
		...
		/**
		 * Function handler for freshizer
		 * It is called only if something happens in the freshzer
		 */
		public function handlerFunction(){

			/* Do something only in the freshizer */

			if( $this->_isInFreshizer  ) {
				$error = error_get_last();
				if (null !== $error) {
					echo '<div style="min-width: 320px !important; background-color: #ffcccc !important;; color: #000000 !important; padding: 100px 40px !important;">';
					echo '<br /><br />';
					echo '<h2>PHP Fatal Error</h2>';
					echo '<br /><br />';
					echo '<p>A fatal error has occured during the attempt to resize the "<a href="'.$this->_url.'" target="_blank">'.$this->_url.'</a>" image.</p>';
					echo '
						<p>Possible causes and their solutions (in the order of occurence) are:</p>
						<ol>
							<li>
								Your webserver has low PHP memory limit.
								<br>
								<strong>Locate "php.ini" file on your webserver and change "memory_limit" variable to at least "128MB" or ideally "256MB". If you do not know how this works, ask your hosting company to do this for you.</strong>
								<br><br>
							</li>
							<li>
								Your webserver does not have the "GD" PHP extension installed or is using a wrong version that is incompatible with the version of PHP that your webserver is running. It is a necessary PHP extension that allows image resizing.
								<br>
								<strong>Ask your hosting company to install the "GD" PHP extension on your webserver.</strong>
								<br><br>
							</li>
							<li>
								The image that you have uploaded and tried to resize is too large. This can be stressful on your webserver.
								<br>
								<strong>Reupload the image with smaller dimensions and size. We don\'t recommend uploading images larger than 2000x2000px and with size no more than 2MB.</strong>
								<br><br>
							</li>
							<li>
								You are resizing the image to a very large resolution like 2000x2000px and larger. This can be stressful on your webserver.
								<br>
								<strong>Do not resize images to resolutions larger than 2000x2000px.</strong>
								<br><br>
							</li>
							<li>
								The <code>/wp-content/uploads/freshizer/</code> directory does not exist on your webserver.
								<br>
								<strong>Using your favorite FTP program, manually create this directory and set it\'s permissions to 0644.</strong>
								<br><br>
							</li>
							<li>
								Your webserver has (almost) no disk space left.
								<br>
								<strong>Double-check if there is enough free space left on your webserver disk.</strong>
								<br><br>
							</li>
						</ol>
						<br>
		...
		/**
		 * Register shutdown function if it is not registered yet
		 */
		public function registerHandler( $url ){

			/* Set that we are doing freshizing */

			$this->_isInFreshizer = true;

			/* Set image URL while freshizing */
			$this->_url = $url;

			/* It needs to be registered just once */

			if( $this->_isRegistered ){
				return;
			}

			register_shutdown_function( array($this, 'handlerFunction') );

			$this->_isRegistered = true;
		}

		public function unregisterHandler(){
			$this->_isInFreshizer = false;
		}
	...
	class fImg
	{
		...
		public static function getImageData($url, $width, $height = false, $crop = false, $delivery = false)
		{
			$width = (int)$width;
			$height = (int)$height;

			return self::getInstance()->_getImageData( $url, $width, $height, $crop, $delivery );
		}

		public static function resize($url, $width, $height = false, $crop = false, $returnImgSize = false, $resizeMultiThread = false, $quality = 90 )
		{
			$width = (int)$width;
			$height = (int)$height;

			$handling = freshizerErrorHandler::getInstance();

			$handling->registerHandler( $url );

			$ret = self::getInstance()->_resize($url, $width, $height, $crop, $returnImgSize, $resizeMultiThread, $quality);

			$handling->unregisterHandler();
			return $ret;
		}
		...
		public function _getImgSize($url)
		{
			...
			if (file_exists($imgData->old->path))
				return getimagesize($imgData->old->path);
			return false;
		}
		...
		private function _isTypeSupported( $url ) {
			$pathInfo = pathinfo( $url );
		...
		public function _resize($url, $width, $height = false, $crop = false, $returnImgSize = false, $quality = 90 )
		{
			if(  defined('FF_ARK_THEME_VERSION') && class_exists('ffThemeOptions')) {
			...
			if( !$this->_isTypeSupported($url ) ) {
				return $url;
			}



			$imgData = $this->_getImgData($url, $width, $height, $crop, $quality);
			$imgData = $this->_getImgDeliverer()->deliveryImage($imgData);
		 ...
		 * @return fImgData
		 */
		private function _getImgData($url, $width, $height, $crop, $quality = 90)
		{
			$imgData = $this->_getImageInfoAsClass($url, $width, $height, $crop);
			$imgData->new->url = $this->_getImgNamer()->getNewImageUrl($url, $width, $height, $crop, false, $quality);
			$imgData->new->filename = $this->_getImgNamer()->getNewImageName($url, $width, $height, $crop, false, $quality);
			$imgData->new->quality = $quality;
